...
def resolve_training_device(config):
    """Resolve the SB3 device string with a safe CUDA probe."""
    ...
    if requested_device in {"auto", "cuda", "gpu"}:
        ...
        if requested_device in {"cuda", "gpu"}:
            print("[Startup] Requested CUDA, but no usable CUDA device was detected; falling back to CPU", flush=True)
        return "cpu"
...
class LinearCurriculumCallback(BaseCallback):
    def __init__(self, sampler, total_timesteps, config, verbose=1):
        ...
        self.lr_end   = config["ppo"].get("learning_rate_end", self.lr_start * 0.1)
        self.ent_start = config["ppo"].get("ent_coef", 0.05)
        self.ent_end   = config["ppo"].get("ent_coef_end", 0.01)

        # Guard against lr_end >= lr_start (e.g. user sets lr=1e-5 but forgets lr_end)
        if self.lr_end >= self.lr_start:
            print(
                f"[Curriculum] WARNING: lr_end ({self.lr_end:.2e}) >= lr_start ({self.lr_start:.2e}). "
                f"Clamping lr_end to lr_start * 0.1.",
                flush=True,
            )
    ...
    def _cosine_anneal(start: float, end: float, progress: float) -> float:
        ...
        return end + 0.5 * (start - end) * (1.0 + np.cos(np.pi * progress))
    ...
    def _on_step(self) -> bool:
        ...
        # 2) Update LR (cosine annealing)
        current_lr = self._cosine_anneal(self.lr_start, self.lr_end, progress)
        for param_group in self.model.policy.optimizer.param_groups:
            param_group["lr"] = current_lr
        self.model.lr_schedule = lambda _: current_lr  # keep SB3's internal schedule in sync

        # 3) Update entropy (cosine annealing)
        current_ent = self._cosine_anneal(self.ent_start, self.ent_end, progress)
...
FLAT_OBS_TAB_COMPONENT_SIZES = (8, 8, 40, 1, 1)


class FlattenGraphObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env, use_action_masks=True):
        super().__init__(env)
        self._component_order = FLAT_OBS_COMPONENT_ORDER
        self._component_sizes = tuple(
            int(np.prod(np.asarray(self.env.observation_space.spaces[key].shape)))
            for key in self._component_order
        )
        self._flat_obs_size = int(sum(self._component_sizes))
    ...
    def action_masks(self):
        if self.use_action_masks and hasattr(self.env, 'action_masks'):
...
class CustomGraphLayoutExtractorV2(BaseFeaturesExtractor):
    """
    Feature extractor for flattened graph observations.

    Flattening before SB3 keeps the rollout buffer on the fast Box path while
    this module reconstructs the pixel and tabular branches on the GPU.
    """
    def __init__(
        self,
        ...
        pixel_map_shape: tuple[int, int, int] = (3, 63, 63),
        ...
        self.pixel_map_shape = tuple(pixel_map_shape)
        self.pixel_map_size = int(np.prod(self.pixel_map_shape))
        self.tab_component_sizes = tuple(int(size) for size in FLAT_OBS_TAB_COMPONENT_SIZES)
        self.component_sizes = (self.pixel_map_size, *self.tab_component_sizes)
        self.flat_obs_size = int(sum(self.component_sizes))

        if observation_space.shape is not None and int(np.prod(observation_space.shape)) != self.flat_obs_size:
            raise ValueError(
                f"Flat observation size mismatch: expected {self.flat_obs_size}, got {observation_space.shape}"
            )

        self.cnn = SpatialCarefulCNN(
            in_channels=self.pixel_map_shape[0],
        ...
        flat_dim = int(sum(self.tab_component_sizes))
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        pixel_flat, cross_map, cross_map_local, local_view, local_crossings, global_crossings = observations.split(
            self.component_sizes,
            dim=1,
        )

        pixel_map = pixel_flat.reshape(-1, *self.pixel_map_shape)
        cnn_out = self.cnn(pixel_map)

        tab_in = torch.cat(
            [cross_map, cross_map_local, local_view, local_crossings, global_crossings],
            dim=1,
        )
...
def make_env(graph_idx, config, sampler, seed=0, flatten_observations=True, use_action_masks=True):
    def _init():
        ...
        if flatten_observations:
            env = FlattenGraphObservationWrapper(env, use_action_masks=use_action_masks)
...
def main():
    ...
    ba = load_split_dataset("train", dataset_type="extended_BA")
    graphs = rome + ba
    print(f"[Startup] Dataset ready: {len(graphs)} graphs", flush=True)
    ...
    flatten_observations = bool(extractor_cfg.get("flatten_observations", True))
    use_action_masks = bool(config["training"].get("use_action_masks", True))
    training_device = resolve_training_device(config)
    patch_size = int(config.get("env", {}).get("patch_size", 63))
    print(f"[Startup] Creating SubprocVecEnv with n_envs={n_envs}...", flush=True)
    vec_env = SubprocVecEnv([
        make_env(i, config, sampler, seed=args.seed, flatten_observations=flatten_observations, use_action_masks=use_action_masks)
        for i in range(n_envs)
    ])
    vec_env = VecMonitor(vec_env)
    vec_env = VecNormalize(vec_env, norm_obs=False, norm_reward=True, clip_reward=10.0)
    print("[Startup] VecEnv, VecMonitor and VecNormalize ready", flush=True)
    ...
    if flatten_observations and use_action_masks:
        policy_class = MaskableActorCriticPolicy
        algo_class = MaskablePPO
    elif flatten_observations:
        policy_class = ActorCriticPolicy
        algo_class = PPO
    elif use_action_masks:
        policy_class = MaskableMultiInputActorCriticPolicy if config["ppo"].get("policy") == "MultiInputPolicy" else config["ppo"]["policy"]
        algo_class = MaskablePPO
    else:
        policy_class = MultiInputActorCriticPolicy if config["ppo"].get("policy") == "MultiInputPolicy" else config["ppo"]["policy"]
        algo_class = PPO
    ...
    policy_kwargs = dict(
        ...
        features_extractor_kwargs=dict(
            ...
            pixel_map_shape=(3, patch_size, patch_size),
            cnn_channels=tuple(extractor_cfg.get("cnn_channels", [32, 64, 128])),
            cnn_blocks_per_stage=tuple(extractor_cfg.get("cnn_res_blocks", [2, 2, 2])),
    ...
    model = algo_class(
        ...
        clip_range_vf=config["ppo"].get("clip_range_vf", config["ppo"]["clip_range"]),
    ...
    if torch.cuda.is_available():
        print(f"[Startup] CUDA available: True, device count: {torch.cuda.device_count()}", flush=True)
        print(f"[Startup] Current CUDA device: {torch.cuda.current_device()}, device name: {torch.cuda.get_device_name()}", flush=True)
        print(f"[Startup] CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', 'not set')}", flush=True)
    else:
        print(f"[Startup] CUDA available: False", flush=True)
    ...
    def _save_profile_and_exit(signum, frame):
        if profiler is not None:
            ...
            print(f"\nReceived signal {signum}. No profiler active, just exiting.")
        sys.exit(0)
    ...
    try:
        ...
        print(f"[Save] Could not save VecNormalize: {e}")
